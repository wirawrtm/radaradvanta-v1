...
interface AddLotModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (lotData: { lot: string; hybrid: string; crops: string; date: string; qty: number }) => Promise<void>;
  isSaving: boolean;
  initialLotNo: string;
}

const AddLotModal = ({ isOpen, onClose, onSave, isSaving, initialLotNo }: AddLotModalProps) => {
  const [lot, setLot] = useState("");
  const [hybrid, setHybrid] = useState("");
  const [crops, setCrops] = useState("Corn");
  const [date, setDate] = useState("");
  const [qty, setQty] = useState(0);

  useEffect(() => {
    if (isOpen) {
      setLot(initialLotNo.trim().toUpperCase());
      setHybrid("");
      setCrops("Corn");
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!lot.trim() || !hybrid.trim() || !crops.trim()) return;
    onSave({ lot, hybrid, crops, date, qty });
  };
        ...
        <div className="p-6 pb-4 border-b border-[#edecff] flex items-center gap-3 bg-white">
          ...
          <div>
            ...
            <p className="text-[10px] text-[#8E94B7] font-bold uppercase tracking-wider mt-0.5">
              Registrasi nomor batch ke database
            </p>
          </div>
        </div>

        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar bg-white">
          <div>
            <label className="text-[10px] text-[#8E94B7] font-bold uppercase tracking-wider ml-1 mb-1.5 block">
              Nomor LOT / Batch
            </label>
            <input
              ...
              className="w-full h-12 bg-[#fbf8ff] border border-[#edecff] focus:border-primary focus:ring-1 focus:ring-primary/10 rounded-full px-5 text-xs font-semibold text-[#111] outline-none transition-all"
              required
              placeholder="Contoh: LOT12345"
            />
          </div>

          <div>
            <label className="text-[10px] text-[#8E94B7] font-bold uppercase tracking-wider ml-1 mb-1.5 block">
              Hybrid (Nama Produk / Deskripsi)
            </label>
            <input
              type="text"
              value={hybrid}
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button
              type="submit"
              disabled={isSaving || !lot.trim() || !hybrid.trim()}
              className="flex-[2] py-3 bg-gradient-to-r from-primary to-cyan-400 text-white rounded-full font-semibold text-xs uppercase tracking-wider shadow-[0_4px_12px_rgba(21,75,226,0.25)] active:scale-[0.98] transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
...
const Dashboard = ({
  ...
  const [addLotModal, setAddLotModal] = useState({
    isOpen: false,
    initialLotNo: "",
  });
  ...
  const handleSaveLot = async (lotData: { lot: string; hybrid: string; crops: string; date: string; qty: number }) => {
    ...
    try {
      ...
      const res = await resp.json();
      if (res.status === "success") {
        setAddLotModal({ isOpen: false, initialLotNo: "" });
        // Set the lotNo state so the stock tab will automatically fetch it and find it
        setLotNo(lotData.lot);
        alert(res.message || "LOT berhasil didaftarkan");
      } else {
        alert("Gagal mendaftarkan LOT: " + (res.message || "Unknown error"));
      }
    } catch (e: any) {
      console.warn("Gagal mendaftarkan LOT", e);
      alert("Terjadi kesalahan saat mendaftarkan LOT: " + e.message);
    } finally {
      setIsActionLoading(false);
    }
  };
    ...
    <div className="w-full max-w-2xl mx-auto lg:max-w-5xl xl:max-w-6xl px-5 pb-8 relative">
                ...
                <div className="space-y-4">
                  ...
                  {!isLotChecking && isLotNotFound && (
                    <div className="space-y-2">
                      ...
                      <button
                        type="button"
                        onClick={() => setAddLotModal({ isOpen: true, initialLotNo: lotNo })}
                        className="w-full py-2.5 bg-red-100 text-red-700 rounded-full font-bold text-[10px] uppercase tracking-wider hover:bg-red-200 transition-all flex items-center justify-center gap-1.5 cursor-pointer border border-red-200"
                      >
                        <span className="material-symbols-outlined text-[14px]">
                          add_circle
                        </span>
                        Daftarkan LOT Baru ke Database
                      </button>
                    </div>
                  )}
      ...
      {activeTab === "partner" && (
        <div className="animate-in fade-in slide-in-from-right-4 duration-300">
          ...
          />
          <AddLotModal
            isOpen={addLotModal.isOpen}
            onClose={() => setAddLotModal({ isOpen: false, initialLotNo: "" })}
            onSave={handleSaveLot}
            isSaving={isActionLoading}
            initialLotNo={addLotModal.initialLotNo}
          />
        </div>
      )}
